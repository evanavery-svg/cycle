...
- **🔐 App lock** — optional PIN lock on launch (and when returning from the background), with a 10-minute grace window so quick reopens don't re-prompt. A screen lock, not full encryption — see `SECURITY.md`.
...
**v0.14** — The version is shown in the app footer and is set by the `APP_VERSION` constant in `js/app.js`. It is bumped on every release (along with the asset cache-busting query strings and the service-worker cache name).
